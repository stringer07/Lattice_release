 ...
 **系统要求:** macOS 14.0 (Sonoma) 或更高版本。专为原生 macOS 体验打造，同时保持轻量与流畅。
...
### 6. 轻量 PDF 连接与自动整理磁盘文件

![自动整理](./lattice_intro/8.png)

Lattice 对 PDF 的处理方式非常克制，它不会把您的 PDF 复制一份并隐藏在应用内部的沙盒数据库里，相反，它仅仅是创建一个指向该附件的链接，这完全不会增加 Lattice 的体积占用。

但这不代表它不管您的文件——**Lattice 可以帮助您在磁盘上自动整理这些 PDF**。
您可以在 `设置` -> `General` 中指定一个基础路径（例如 iCloud 或其它网盘文件夹以实现同步），并选择整理策略（是复制一份还是移动到目标路径）。Lattice 会自动根据文献的年份、文集等属性，为您创建子文件夹并有条理地归类存放这些 PDF。

这意味着，您的文件结构依然清晰、高度可控，且完全属于您自己。您可以在 Lattice 中:

- 打开 PDF
- 在 Finder 中定位 PDF
- 替换关联 PDF

这对需要掌控本地文献库的人尤其友好。Lattice 帮助你自动化整理目录结构，而不是要求你重新迁移进一个封闭系统。
